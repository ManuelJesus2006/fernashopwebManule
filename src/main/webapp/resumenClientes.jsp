...
<div class="contenedor-registros">
    <%ArrayList<Cliente> clientesRegistrados = controlador.getClientes();
        if (clientesRegistrados.isEmpty()){%>
    <h2 class="no-registros">No se han registrado clientes</h2>
    <%}else{
        for (Cliente c : clientesRegistrados){%>
    <div class="tarjeta-registro">
        <h2>Cliente nº<%=c.getId()%></h2>
        <h3>Nombre: <%=c.getNombre()%></h3>
        <h3>Localidad: <%=c.getLocalidad()%> (<%=c.getProvincia()%>)</h3>
        <h3>Correo: <%=c.getEmail()%></h3>
        <h3>Móvil: <%=c.getMovil()%></h3>
    </div>
    <%}
    }%>
</div>
